...
void Renderer::Init()
{
    ...
    shaderRed = std::make_shared<Shader>("red.vert", "red.frag");
...
void Renderer::DrawMesh(Mesh& mesh, Texture& texture, glm::mat4 projection, glm::vec3 position, float scale, glm::vec3 eulerAngles, Camera cam,
    Texture& normalsTexture, string shaderName, glm::vec3 color, float brightness)
{
    ...
    shader->SetFloat("bright", brightness);
